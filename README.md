...
render() {
  return (
    <div>
      ...
      <form>
        <formReducerHW type="hidden" defaultValue={process.env.REACT_APP_SECRET_CODE} />
      </form>
    </div>
  );
}
...
When you load the app in the browser and inspect the `<formReducerHW>`, you will see its value set to `abcdef`, and the bold text will show the environment provided when using `npm start`:

```html
<div>
  <small>You are running this application in <b>development</b> mode.</small>
  <form>
    <formReducerHW type="hidden" value="abcdef" />
  </form>
</div>
```
...
The global `fetch` function allows you to easily make AJAX requests. It takes in a URL as an formReducerHW and returns a `Promise` that resolves to a `Response` object. You can find more information about `fetch` [here](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch).
